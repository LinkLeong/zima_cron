...
function onRowAction(e){const action=e.currentTarget.getAttribute('data-action');const id=e.currentTarget.getAttribute('data-id');const task=tasks.find(t=>t.id===id);if(!task)return;if(action==='run')fetch(`${API_BASE}/zimaos_cron/tasks/${id}/run`,{method:'POST'}).then(fetchTasks);if(action==='toggle')fetch(`${API_BASE}/zimaos_cron/tasks/${id}/toggle`,{method:'POST'}).then(fetchTasks);if(action==='logs'){task.showLogs=!task.showLogs;renderTasks()}if(action==='clear-logs'){fetch(`${API_BASE}/zimaos_cron/tasks/${id}/logs/clear`,{method:'POST'}).then(()=>{task.logs=[];renderTasks()})}if(action==='delete'){fetch(`${API_BASE}/zimaos_cron/tasks/${id}`,{method:'DELETE'}).then(fetchTasks)}}
function runAllOnce(){const running=tasks.filter(t=>t.status==='running');Promise.all(running.map(t=>fetch(`${API_BASE}/zimaos_cron/tasks/${t.id}/run`,{method:'POST'}))).then(fetchTasks)}
